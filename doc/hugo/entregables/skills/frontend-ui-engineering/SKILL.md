...
> **Nota para proyectos Hugo:** Los ejemplos de código usan React/TSX. Para Hugo, aplicar los mismos principios a plantillas Go (layouts, partials, shortcodes). Los conceptos de arquitectura de componentes, diseño responsivo, accesibilidad y gestión de estados son universales.
...
> **Hugo:** Estructura equivalente — `layouts/partials/tasklist.html` + `layouts/shortcodes/tasklist.html` + assets CSS/JS asociados.
...
> **Hugo:** La gestión de estado en Hugo es diferente — los datos vienen de front matter, data files, o APIs externas vía `getJSON`. Usar `.Scratch` para estado temporal en plantillas.
