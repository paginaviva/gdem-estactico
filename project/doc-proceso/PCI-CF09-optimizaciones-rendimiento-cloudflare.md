...
El proyecto GDEM (Gaia Demurtas, `gaiaevoluciondelser.es`) consiste en la migración de un sitio WordPress a un sitio estático generado con **Hugo**, desplegado en **Cloudflare Pages** (plan Free). Como parte de la **Etapa 4** del plan de trabajo (`etapa-4-pdTbjo-despliegue-cloudflare.md`), la tarea **CF09** consiste en activar las optimizaciones de rendimiento a nivel de zona Cloudflare (settings de dominio) para maximizar la velocidad de carga del sitio estático.
